...
import type { MarketChangeLogEntry } from '../../types';

export function MarketChangeLog({
  changes,
  onClose,
}: {
  changes: MarketChangeLogEntry[];
  onClose: () => void;
}) {
  return (
      ...
      <div className="bg-white rounded-lg max-w-6xl w-full max-h-[90vh] overflow-hidden shadow-xl">
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex justify-between items-center">
            <h2 className="text-lg font-semibold text-retool-heading">Market Change Log</h2>
            <button
              onClick={onClose}
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {changes.map((entry, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-900">{entry.marketName}</td>
